...
public class BookServiceImpl implements BookService {
    ...
    private Book setBookData(Book editedBook, Book currentBook) {
        Optional<Author> foundAuthor = authorRepository.findById(editedBook.getAuthor().getId());
        if (foundAuthor.isPresent()) {
            currentBook.setAuthor(foundAuthor.get());
        } else {
            currentBook.getAuthor().setFio(editedBook.getAuthor().getFio());
        }
        Optional<Genre> foundGenre = genreRepository.findById(editedBook.getGenre().getId());
